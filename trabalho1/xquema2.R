...
v <- u
#j=2
for(tt in (1:(nt-1))){ # tempo
  #for(j in (2:(nd-1))){
  for(i in (1:(nd-1))){ # espaço 'x'
      #if(i==1&&tt==1){
      #b1 <- as.matrix(c(1-2*fo,fo))
      ...
      #c1 <- c(analitica(D,L,uo,xi[1:nd],y = yi[1]-diff(yi)[1],t=tt+1))
      #c2 <- c(analitica(D,L,uo,x=xi[i],y = yi[1]-diff(yi)[1],t=tt+2),rep(0,nd-2),
      #        analitica(D,L,uo,x=xi[i],y =yi[1]-diff(yi)[1],t=tt+2))
      #c3 <- matrix(c(c1,c2),ncol=2)
      #c4 <- matrix(c(fo,fo))
      #c5 <- c3%*%c4
      #
      #aux <- A%*%v[,i,tt+2]-c5-b1c
      #v[,i+1,tt+1] <- aux/fo
    }
    if(i==nd){
      ...
      c1 <- as.matrix(c(analitica(D,L,uo,xi[1],y = yi[1]-diff(yi)[1],t=tt+2),rep(0,nd-2),
                        analitica(D,L,uo,xi[nd],y = yi[1]-diff(yi)[1],t=tt+2)))
      c2 <- as.matrix(analitica(D,L,uo,xi[1:nd],y = yi[1]-diff(yi)[1],t=tt+1))
      c3 <- matrix(c(c2,c1),ncol=2)
      c4 <- as.matrix(c(fo,fo))
      c5 <- c3%*%c4
      
      
      v[,i,tt+2] <- solve(A,t(B)+c5)
    }
    if(i>1&&i<nd){
      ...
      c1 <- as.matrix(c(analitica(D,L,uo,xi[i],y = yi[1]-diff(yi)[1],t=tt+2),rep(0,nd-2),
                      analitica(D,L,uo,xi[i],y = yi[1]-diff(yi)[1],t=tt+2)))
      c5 <- fo*c1
